...
describe("HubPoolClient: RootBundle Events", function () {
  ...
  it("gets most recent CrossChainContractsSet event for chainID", async function () {
    ...
    expect(hubPoolClient.getSpokePoolForBlock(11, firstUpdateBlockNumber).formatAsNativeAddress()).to.equal(spokePool1);
    expect(hubPoolClient.getSpokePoolForBlock(11, secondUpdateBlockNumber).formatAsNativeAddress()).to.equal(
      spokePool2
    );
