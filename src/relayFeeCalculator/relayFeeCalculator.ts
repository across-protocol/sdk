...
export class RelayFeeCalculator {
  ...
  async gasFeePercent(
    ...
    const destinationChainTokenDetails = Object.values(tokenMapping).find(
      (details) =>
        compareAddressesSimple(details.addresses[originChainId], inputToken.formatAsNativeAddress()) &&
    ...
    const outputToken = isZeroAddress(deposit.outputToken)
      ...
      : deposit.outputToken.formatAsNativeAddress();
    const outputTokenInfo = getTokenInfo(outputToken, destinationChainId, tokenMapping);
    const inputTokenInfo = getTokenInfo(inputToken.formatAsNativeAddress(), originChainId, tokenMapping);
  ...
  async relayerFeeDetails(
    ...
    const inputTokenInfo = getTokenInfo(inputToken.formatAsNativeAddress(), originChainId);
    const outputTokenInfo = getTokenInfo(outputToken.formatAsNativeAddress(), destinationChainId);
