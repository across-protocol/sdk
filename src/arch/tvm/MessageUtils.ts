 ...
 *
 * `calldata` is measured directly from the real populated transaction rather than
 * modeled from a per-function ABI layout: this way the estimate stays correct for any
 * TVM call shape (a plain `fillRelay`, `fillRelayWithUpdatedDeposit`, or anything else)
 * without needing its own hand-counted calldata-size constant.
 *
