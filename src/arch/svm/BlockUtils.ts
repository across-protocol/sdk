...
export class SVMBlockFinder extends BlockFinder<SVMBlock> {
  ...
  // To mitigate this, `getBlock` returns the nearest block greater than or equal to the input number.
  private async getBlock(number: number): Promise<SVMBlock> {
    ...
    const blocks = await this.provider.getBlocks(BigInt(number), BigInt(number + defaultBlockRange)).send();
    const _block = await this.provider.getBlock(blocks[0]).send();
