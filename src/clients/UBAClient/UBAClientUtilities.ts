...
import { UbaFlowWithFees, getRefundFee } from "../../UBAFeeCalculator/UBAFeeSpokeCalculatorAnalog";
import {
...
export function getFeesForFlow(
  flow: UbaFlow,
  precedingFlowsInBundle: ModifiedUBAFlow[],
  ...
  // Compute the balancing fees for the flow. Both deposit and refund balancing fees are based on
  // the opening balance of the bundle containing the flow.
  const matchedDeposit: DepositWithBlock = isUbaOutflow(flow) ? flow.matchedDeposit : flow;
  const l1TokenAddress = hubPoolClient.getL1TokenForDeposit(matchedDeposit);
  // We can always compute this opening balance for the deposit deterministically since it only
  // fetches from on-chain published running balances.
  const openingBalanceForDeposit = hubPoolClient.getRunningBalanceBeforeBlockForChain(
    matchedDeposit.blockNumber,
    matchedDeposit.originChainId,
    l1TokenAddress
  );
  const bundleStartBlockForDeposit = hubPoolClient.getBundleStartBlockContainingBlock(
    matchedDeposit.blockNumber,
    matchedDeposit.originChainId,
    hubPoolClient.latestBlockNumber
  );
  const ubaConfigForDeposit = getUBAFeeConfig(
    hubPoolClient,
    matchedDeposit.originChainId,
    tokenSymbol,
    bundleStartBlockForDeposit
  );
  // To get correct balancing fees use the latest incentive pool balance as of the latest preceding flow:
  const latestIncentiveBalance =
    precedingFlowsInBundle.length > 0
      ? precedingFlowsInBundle[precedingFlowsInBundle.length - 1].incentiveBalance
      : bundleState.openingIncentiveBalance;

  const { balancingFee: depositBalancingFee } = analog.getDepositFee(
    matchedDeposit.amount,
    openingBalanceForDeposit.runningBalance,
  ...
  // Now, assuming this flow is valid and all preceding flows are valid, compute the resultant running balance for this
  // flow.
  const newFlow: UbaFlowWithFees = {
    flow,
    systemFee: {
      depositBalancingFee,
      lpFee,
      systemFee: depositBalancingFee.add(lpFee),
    },
    relayerFee: {
      relayerBalancingFee,
    },
...
export async function getModifiedFlow(
  ...
  const {
    ...
    lastIncentiveBalance,
    netRunningBalanceAdjustment,
  } = getFeesForFlow(flow, precedingFlows, bundleData, chainId, tokenSymbol, hubPoolClient);
  return {
    flow,
    systemFee: {
