...
export async function readEvents<IDL extends Idl = Idl>(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return processEventFromTx(txResult as any, programs);
}
...
export async function readEventsUntilFound<IDL extends Idl = Idl>(
  ...
  while (Date.now() - startTime < 5000) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (txResult !== null) return processEventFromTx(txResult as any, programs);
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const depositIdSchema: any = new Map(
