...
import { SvmSpokeAnchor as SvmSpoke } from "../assets";
...
export async function closeInstructionParams(program: Program<SvmSpoke>, signer: Keypair) {
  ...
  if (accountInfo !== null) {
    ...
    await sendAndConfirmTransaction(program.provider.connection as any, new Transaction().add(closeIx), [signer]);
  }
}
...
): Promise<{ loadInstructions: TransactionInstruction[]; closeInstruction: TransactionInstruction }> {
  const maxInstructionParamsFragment = 900; // Should not exceed message size limit when writing to the data account.
